...
export default function SettingsPage() {
  ...
  return (
    <div className="space-y-5 sm:space-y-6 animate-fade-in max-w-2xl">
      <div>
        <h1 className="text-xl sm:text-2xl font-bold text-hermes">Configuracion</h1>
        <p className="text-xs sm:text-sm text-muted mt-1">Ajustes del sistema Hermes Mission Control</p>
      </div>

      <Section title="Conexion al Servidor" icon="M5.25 14.25h13.5m-13.5 0a3 3 0 01-3-3m3 3a3 3 0 100 6h13.5a3 3 0 100-6m-16.5-3a3 3 0 013-3h13.5a3 3 0 013 3m-19.5 0a4.5 4.5 0 01.9-2.7L5.737 5.1a3.375 3.375 0 012.7-1.35h7.126c1.062 0 2.062.5 2.7 1.35l2.587 3.45a4.5 4.5 0 01.9 2.7m0 0a3 3 0 01-3 3m0 3h.008v.008h-.008v-.008zm0-6h.008v.008h-.008v-.008zm-3 6h.008v.008h-.008v-.008zm0-6h.008v.008h-.008v-.008z">
        <Field label="IP del servidor"><input type="text" value={serverIp} onChange={(e) => setServerIp(e.target.value)} className="input-field" /></Field>
        <Field label="Base path API"><input type="text" value={apiBase} onChange={(e) => setApiBase(e.target.value)} className="input-field" /></Field>
        <Field label="Intervalo de refresco (seg)"><input type="number" value={refreshInterval} onChange={(e) => setRefreshInterval(e.target.value)} className="input-field" /></Field>
      </Section>

      <Section title="API de NVIDIA" icon="M15.75 5.25a3 3 0 013 3m3 0a6 6 0 01-7.029 5.912c-.563-.097-1.159.026-1.563.43L10.5 17.25H8.25v2.25H6v2.25H2.25v-2.818c0-.597.237-1.17.659-1.591l6.499-6.499c.404-.404.527-1 .43-1.563A6 6 0 1121.75 8.25z">
        ...
        <p className="text-[11px] text-muted leading-relaxed">Autenticacion con modelos NVIDIA NIM en el servidor.</p>
      </Section>

      <Section title="MCP Endpoint" icon="M13 10V3L4 14h7v7l9-11h-7z">
        <div className="bg-surface/80 rounded-xl p-3 border border-card-border">
          <p className="text-[11px] text-muted mb-1">URL del MCP Server</p>
      ...
      <Section title="Preferencias" icon="M10.5 6h9.75M10.5 6a1.5 1.5 0 11-3 0m3 0a1.5 1.5 0 10-3 0M3.75 6H7.5m3 12h9.75m-9.75 0a1.5 1.5 0 01-3 0m3 0a1.5 1.5 0 00-3 0m-3.75 0H7.5m9-6h3.75m-3.75 0a1.5 1.5 0 01-3 0m3 0a1.5 1.5 0 00-3 0m-9.75 0h9.75">
        <label className="flex items-start gap-3 cursor-pointer p-3 bg-surface/50 rounded-xl hover:bg-surface/80 transition-colors">
          ...
          <div>
            ...
            <p className="text-[11px] text-muted">Alertas cuando un agente falle o cambie de estado</p>
          </div>
        </label>
        <label className="flex items-start gap-3 cursor-pointer p-3 bg-surface/50 rounded-xl hover:bg-surface/80 transition-colors">
          ...
          <div>
            ...
            <p className="text-[11px] text-muted">Reconectar si se pierde conexion con el servidor</p>
          </div>
        </label>
      </Section>

      <div className="flex items-center gap-3 pt-1">
        <button onClick={handleSave} className="px-5 py-2.5 bg-gold/15 text-gold-light text-sm font-medium rounded-xl border border-gold/20 hover:bg-gold/25 transition-all active:scale-95">
